...
import { addSetSchema, createWorkoutSessionSchema, createWorkoutTemplateSchema, updateWorkoutStatusSchema } from "../schemas/workout.schema";
import { addSet, createSession, createTemplate, getSessionsForUser, getTemplates, updateSessionStatus } from "../services/workout.service";
...
export const getTemplatesHandler = async (req: Request, res: Response) => res.status(200).json(await getTemplates(req.user!.id));
...
export const getSessionsHandler = async (req: Request, res: Response) => res.status(200).json(await getSessionsForUser(req.user!.id));
