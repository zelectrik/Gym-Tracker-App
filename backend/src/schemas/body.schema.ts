...
export const createBodyGoalSchema = z.object({
  ...
  const isMaintain = data.goalType === "MAINTAIN_ABOVE" || data.goalType === "MAINTAIN_BELOW";
  return {
    ...data,
  ...
  (data) => data.goalType === "MAINTAIN_ABOVE" || data.goalType === "MAINTAIN_BELOW" || data.level >= 1,
  { message: "Le niveau est obligatoire pour un objectif de perte ou de progression." },
);
