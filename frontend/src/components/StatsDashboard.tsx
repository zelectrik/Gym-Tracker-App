...
import type { Exercise, ExerciseSet, SessionExercise, WorkoutSession } from "../types";
...
function buildExerciseMetrics(sessions: WorkoutSession[], exerciseId: string) {
  return sessions
    ...
    .flatMap((session) => {
      const matchingExercises = session.exercises.filter((exercise) => exercise.exerciseId === exerciseId);
      return matchingExercises.map((exercise) => ({ session, exercise }));
    })
    .map(({ session, exercise }) => {
      const sets = getCompletedSets(exercise);
      const reps = metricStats(sets.map((set) => set.reps ?? 0).filter(Boolean));
      const weights = metricStats(sets.map((set) => set.weightKg ?? 0).filter(Boolean));
...
function MiniChart({ title, points, unit, mode = "stats" }: { title: string; points: MetricPoint[]; unit: string; mode?: "stats" | "total" }) {
  const width = 320;
  const height = 150;
  const padding = 24;
  const values = points.flatMap((point) => (mode === "total" ? [point.total] : [point.min, point.max, point.avg]));
  const maxValue = Math.max(...values, 1);
  const x = (index: number) =>
    points.length === 1 ? width / 2 : padding + (index * (width - padding * 2)) / (points.length - 1);
  const y = (value: number) => height - padding - (value / maxValue) * (height - padding * 2);
  const linePath = (selector: (point: MetricPoint) => number) =>
    points.map((point, index) => `${index === 0 ? "M" : "L"}${x(index)},${y(selector(point))}`).join(" ");
  ...
  return (
    <article className="chart-card">
      ...
      {points.length === 0 ? (
        ...
        <>
          <svg className="mini-chart" viewBox={`0 0 ${width} ${height}`} role="img" aria-label={title}>
            <line x1={padding} y1={height - padding} x2={width - padding} y2={height - padding} />
            {mode === "total" ? (
              <path d={linePath((point) => point.total)} className="chart-line chart-total" />
            ) : (
              <>
                <path d={linePath((point) => point.max)} className="chart-line chart-max" />
                <path d={linePath((point) => point.avg)} className="chart-line chart-avg" />
                <path d={linePath((point) => point.min)} className="chart-line chart-min" />
              </>
            )}
            {points.map((point, index) => (
              <circle key={`${point.date}-${index}`} cx={x(index)} cy={y(mode === "total" ? point.total : point.avg)} r="3" />
            ))}
          </svg>
          ...
          <div className="chart-last-value">
            Dernier : {Math.round((points.at(-1)?.[mode === "total" ? "total" : "avg"] ?? 0) * 10) / 10} {unit}
          </div>
        </>
      )}
    </article>
...
function WorkoutSessionDetail({ session, onClose, onSelectExercise }: { session: WorkoutSession; onClose: () => void; onSelectExercise: (exerciseId: string) => void }) {
  const completedSets = session.exercises.flatMap((exercise) => getCompletedSets(exercise));
  const totalVolume = completedSets.reduce((sum, set) => sum + getSetVolume(set), 0);
            ...
            <article key={exercise.id} className="session-exercise-detail">
              <div className="session-exercise-head">
                <div>
                  ...
                  <span>{exercise.exercise.trackingType === "CARDIO" ? formatCardio(exercise) : `${sets.length}/${exercise.sets.length} séries`}</span>
                </div>
                <button type="button" onClick={() => onSelectExercise(exercise.exerciseId)}>Voir évolution</button>
              </div>
              ...
              {exercise.exercise.trackingType !== "CARDIO" && (
                <div className="set-chip-list">
                  {sets.map((set) => (
                    <span key={set.id}>
                      S{set.setNumber} {set.side !== "BOTH" ? set.side : ""} · {set.reps ?? "?"} reps · {set.weightKg ?? 0} kg
                    </span>
                  ))}
    ...
    <section className="card exercise-progress-panel">
      ...
      {isCardio ? (
        <div className="cardio-history-list">
          {metrics.map((item) => (
            <article key={`${item.sessionId}-${item.date}`} className="cardio-history-item">
              <strong>{formatDateTime(item.date)}</strong>
              <span>{item.sessionTitle}</span>
              <b>{formatCardio({ cardioEntry: item.cardio } as SessionExercise)}</b>
            </article>
          ))}
        ...
        <div className="chart-grid">
          ...
          <MiniChart title="Volume soulevé" points={volumePoints} unit="kg" mode="total" />
        </div>
      )}
    </section>
  );
...
export function StatsDashboard({ sessions, exercises }: { sessions: WorkoutSession[]; exercises: Exercise[] }) {
  ...
  const exerciseOptions = useMemo(() => {
    const ids = new Set(completedSessions.flatMap((session) => session.exercises.map((exercise) => exercise.exerciseId)));
      ...
      <section className="card workout-calendar-card">
        <div className="section-title">
          <div>
            ...
            <h3>Deux dernières semaines</h3>
            <p>Chaque carte ouvre le détail de la séance.</p>
          </div>
        </div>

        <div className="workout-calendar-grid">
          {getLast14Days().map((day) => {
            const key = dateKey(day.toISOString());
            const daySessions = sessionsByDay.get(key) ?? [];
            return (
              <article key={key} className={`calendar-day-card ${daySessions.length ? "has-session" : "empty"}`}>
                <header>
                  <strong>{formatShortDate(day)}</strong>
                  <span>{daySessions.length ? `${daySessions.length} séance(s)` : "Repos"}</span>
                </header>
